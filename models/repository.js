...
class Repository {
    ...
    getAll(params = null) {
        ...
        if (params) {
            ...
            // todo filter
            // Vérifie si tous les éléments de objectList respectent le filtre donné dans searchKeys[]
            objectsList.forEach(object => {
                if (this.filterObject(object, searchKeys)) {
            ...
            // Voici mon essai de le faire fonctionner avec le .sort.
            // Voir au dessus de la méthode innerCompare plus bas pour plus d'info
            // // true if ascending and false if descending
            // if(sortKeys[0].asc) {
            //     filteredItems.sort(function(x, y){
            //         return innerCompare(x[sortKeys[0].key], y[sortKeys[0].key])
            //     });
            // }
            // else if(!sortKeys[0].asc) {
            //     filteredItems.sort(function(x, y){
            //         return innerCompare(y[sortKeys[0].key], x[sortKeys[0].key])
            ...
            // ne fonctionne pas
            if(sortKeys[0].asc) {
                for (let i = 0; i < filteredItems.length.length; i++) {
                    for (let j = 0; j < filteredItems.length; j++) {
                        if (this.innerCompare(filteredItems[j][sortKeys[0].key], filteredItems[i][sortKeys[0].key])) {
                ...
                for (let i = 0; i < filteredItems.length.length; i++) {
                    for (let j = 0; j < filteredItems.length; j++) {
                        if (this.innerCompare(filteredItems[i][sortKeys[0].key], filteredItems[j][sortKeys[0].key])) {
                            let item =  filteredItems[i];
                            filteredItems[j] = filteredItems[i];
    ...
    // retourne vrai si l'object respecte tous les filtres donnés en paramètre.
    filterObject(object, filters) {
        for(let filter of filters){
            if (!this.valueMatch(object[filter.key], filter.value)) {
                return false;
            }
        }
        return true;
    ...
    // Puisque le array.sort ne fonctionne pas de la façon attendue avec le this,
    // il est impossible d'appeler la méthode this.innerCompare. J'ai essayé de 
    // changer la méthode pour qu'elle puisse être appelée mais en vain.
    // Voici l'essai que j'ai fais : 
    // var innerCompare = function(x, y) {
    //     if ((typeof x) === 'string')
